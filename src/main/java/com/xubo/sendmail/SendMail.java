...
//           将map中的数据拼接到html的table中
            sb.append(sdf.format(map.get(fileName))).append("\t\n").append(fileName+"\n");
        }
        map.clear();
...
//        System.out.println(files.length); // 输出文件的个数
        if (files.length > 0) {
            for (File f : files) {
                if (f.isDirectory()) {
//                    是目录 递归
...
//                            update
    ...
    public static String myEmailAccount = "1837039333@qq.com";
    public static String myEmailPassword = "cpwvzgucsiopbdfe"; // 记住 上传 github 的时候注意把这个删除掉
    ...
    // 收件人邮箱（替换为自己知道的有效邮箱）
    public static String receiveMailAccount = "x183796@163.com";
